...
export default function Home() {
    ...
    return (
        <main className="min-h-screen font-sans selection:bg-blue-500/30 pb-20 overflow-x-hidden relative" style={{ color: 'var(--text-primary)', backgroundColor: 'var(--bg-primary)' }}>

            {/* === GLOBAL MOUSE SPOTLIGHT === */}
            <div
                className="pointer-events-none fixed inset-0 z-30 transition-opacity duration-300"
                style={{
                    background: `radial-gradient(600px at ${mousePosition.x}px ${mousePosition.y}px, rgba(29, 78, 216, ${spotlightOpacity}), transparent 80%)`
                }}
            />
            ...
            <div className="max-w-7xl mx-auto px-6 flex flex-col gap-24 relative z-10">
                ...
                <section className="min-h-[75vh] flex items-center pt-10 relative">
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 items-center w-full">
                        ...
                        >
                            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mb-4 tracking-tight leading-tight" style={{ color: 'var(--text-primary)' }}>
                                Hi, I&apos;m {hero.name.split(" ")[0]}
                            </h1>

                            <p className="text-lg md:text-xl mb-6 leading-relaxed max-w-xl" style={{ color: 'var(--text-secondary)' }}>
                                <span className="highlight-text">{hero.title}</span>{" "}
                                {hero.text}
                            </p>
                            ...
                            <div className="flex flex-wrap gap-4">
                                ...
                                <a href="https://github.com/Bhat-18" target="_blank" className="flex items-center gap-2 px-8 py-3 rounded-full font-bold text-sm md:text-base transition border" style={{ backgroundColor: 'var(--bg-tertiary)', borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}>
                                    <FiGithub /> Github
                                </a>
                            </div>
                        </motion.div>
                ...
                <section>
                    <div className="flex items-center justify-between mb-10">
                        <h2 className="text-4xl md:text-5xl font-black" style={{ color: 'var(--text-primary)' }}>Experience</h2>
                        <Link href="/experience" className="flex items-center gap-2 font-bold transition" style={{ color: 'var(--accent-primary)' }}>Full History <FiArrowRight /></Link>
                    </div>
                    <div className="flex flex-col gap-8">
                        {homeExperience.map((job, index) => (
                            <motion.div
                                ...
                                className="relative pl-6 border-l-2 transition duration-500"
                                style={{ borderColor: 'var(--border-color)' }}
                            >
                                <div className="p-8 rounded-2xl border transition shadow-lg group" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
                                        <h3 className="text-xl md:text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{job.role}</h3>
                                        <span className="text-xs font-bold px-3 py-1 rounded-full w-fit whitespace-nowrap" style={{ color: 'var(--accent-primary)', backgroundColor: 'rgba(59, 130, 246, 0.1)' }}>
                                            {job.date}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-3 mb-6" style={{ color: 'var(--text-secondary)' }}>
                                        ...
                                        <span className="w-1 h-1 rounded-full" style={{ backgroundColor: 'var(--text-dim)' }}></span>
                                        <span className="text-xs md:text-sm font-medium uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>
                                            {job.location}
                                        </span>
                                    </div>
                                    <ul className="space-y-3">
                                        {job.points && job.points.map((point, i) => (
                                            <li key={i} className="flex items-start gap-3 text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                                                <span className="mt-1.5 w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: 'var(--accent-primary)' }}></span>
                                                <span>{point}</span>
                                            </li>
                                        ))}
                                    </ul>
                ...
                <section className="diagonal-lines-bg py-16 -mx-6 px-6 rounded-3xl">
                    <h2 className="text-4xl md:text-5xl font-black text-center mb-16 relative z-10" style={{ color: 'var(--text-primary)' }}>Technology Stack</h2>
                    <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6 md:gap-8 max-w-5xl mx-auto relative z-10">
                        {skills.map((skill, index) => (
                            ...
                            >
                                <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-xl border shadow-xl group-hover:-translate-y-2 transition-all duration-300" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                                    <TechIcon name={skill} size={44} />
                                </div>
                                <span className="text-xs md:text-sm font-medium transition-colors duration-300 text-center" style={{ color: 'var(--text-muted)' }}>
                                    {skill}
                                </span>
                            </motion.div>
                        ))}
                        ...
                        >
                            <div className="w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-xl border border-dashed" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--text-dim)' }}>
                                <span className="text-2xl" style={{ color: 'var(--text-dim)' }}>...</span>
                            </div>
                            <span className="text-xs md:text-sm font-medium text-center" style={{ color: 'var(--text-dim)' }}>
                ...
                <section className="pb-20 overflow-hidden">
                    <div className="flex items-center justify-between mb-10 px-2">
                        <h2 className="text-4xl md:text-5xl font-black" style={{ color: 'var(--text-primary)' }}>Certifications</h2>
                        <Link href="/certifications" className="flex items-center gap-2 font-bold transition" style={{ color: 'var(--accent-primary)' }}>View All <FiArrowRight /></Link>
                    </div>
                        ...
                        >
                            {marqueeCerts.map((cert, i) => (
                                <div key={`${cert.id}-${i}`} className="w-[280px] shrink-0">
                                    <Card3D className="h-full flex flex-col rounded-2xl overflow-hidden group">
                                        <div className="relative h-[160px] w-full border-b p-4 flex items-center justify-center" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
                                            {cert.image ? (
                                                <Image src={cert.image} alt={cert.title} fill className="object-contain" />
                                            ) : (
                                                <FiAward className="text-5xl" style={{ color: 'var(--text-dim)' }} />
                                            )}
                                        </div>
                                        <div className="p-5 flex flex-col gap-2 flex-1">
                                            <h3 className="text-sm font-bold line-clamp-1 group-hover:text-blue-400 transition" style={{ color: 'var(--text-primary)' }}>{cert.title}</h3>
